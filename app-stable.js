      ...
      const processFrame = () => {
        ...
        if (lastProcessedTime < 0 || current - lastProcessedTime >= 1 / 12) {
          ...
          if (tracked && isReliablePose(tracked.landmarks)) {
            ...
            if (!lastAcceptedHip || jump < 0.09) {
              const stableLandmarks = smoothLandmarks(tracked.landmarks);
              frames.push({
                time: current,
                landmarks: stableLandmarks,
                world: tracked.world
              });
              lastAcceptedHip = midpoint(stableLandmarks[23], stableLandmarks[24]);
            }
          }
...
function smoothLandmarks(points) {
  if (!smoothedPose) {
    smoothedPose = points.map(point => ({ ...point }));
    return smoothedPose.map(point => ({ ...point }));
  }

  smoothedPose = points.map((point, index) => {
    const previous = smoothedPose[index] || point;
    const confidence = point.visibility ?? 0.5;

    // Higher-confidence joints respond faster; uncertain joints move slowly.
    const alpha = confidence >= 0.8 ? 0.42 : confidence >= 0.6 ? 0.28 : 0.14;
...
function renderReport(a) {
  ...
  document.querySelector(".scoreRing").style.background =
    "conic-gradient(#45c86c 360deg,#35434d 0deg)";
  document.getElementById("headline").textContent = "Tracked measurements";
  ...
  document.getElementById("primaryIssue").textContent = "Coach review required";
  document.getElementById("why").textContent = "Confirm the skeleton stayed on the hitter before using any measurement.";
  document.getElementById("oneThing").textContent = "Choose one observed movement to address.";
  document.getElementById("drill").textContent = "Select after coach confirmation.";
